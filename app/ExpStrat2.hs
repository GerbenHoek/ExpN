...
test2 = Map.singleton "inter" 137.5 :: Ref Rational
...
propS :: Strat (Ref Rational)
propS = 
   ((calcFactorS .*. 
   R (toggleRule growNoExp)) <|> lin) .*. 
   R (toggleRule dx1xv_Is_xv) .*. 
   R (toggleRule baseIs_1) .*. 
   R newGrow1Mul .*.
   R interB .*.
   finalS 
   where lin = R factorIsdy .*. 
               R calc_dx .*. 
               R growDiv .*. 
               option (R invertGrow) .*. 
