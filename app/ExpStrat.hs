...
test2 = Map.singleton "inter" 73.6 :: Ref Rational
...
calcGrowS = calc_dxS .*.
            ...
            option (R roundGrow3 <|> R roundGrow <|> R roundGrowB)
...
interBS = 
   ...
   --(R baseIs_x1 .*. (R newGrow2 <|> R newGrow2Mul <|> 
   --   R newGrow2 <|> R newGrow2Mul <|> R newGrowIsg)) <|>
   --(R baseIs_x2 .*. (R newGrow2 <|> R newGrow2Mul <|> 
   --   R newGrow2 <|> R newGrow2Mul <|> R newGrowIsg))
   ) .*. 
   (R interB <|> R interAdd)
